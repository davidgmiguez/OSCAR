...
function findObjectsWatershed(binaryImage::Matrix{Int}, useInHouseAlgorithm::Bool=true, returnLabeledImage::Bool=false, excludeBorder::Bool=true)::Union{Array{ImageObject},Matrix{Int}}
    if !useInHouseAlgorithm
        ...
        try
            ...
            # end
            # objSize = median(distances[findall(distances .!=0)])
            # wVal = Int(objSize + ((objSize + 1) % 2))
            # distances = removeEqualNeighbours(distances, 2*objSize)
            distances[findall(x -> x == 0.0, distances)] .= typemin(Float64)

            #Find maximum coords
            # maximumCoords::Vector{CartesianIndex{2}} = findlocalmaxima(distances, window = (wVal,wVal))
            maximumCoords::Vector{CartesianIndex{2}} = findmaxima_inHouse(distances)
...
"""
    get_bounding_box(img::AbstractArray)::BoundingBox

Computes the bounding box of the nonzero elements in a grayscale or binary image. The function identifies all nonzero pixels and determines the minimum and maximum coordinates along each axis.

# Arguments
- `img::AbstractArray`: A 2D grayscale or binary image.

# Returns
- `BoundingBox`: A structure containing the coordinates of the bounding box in the format:
BoundingBox(x_max, x_min, y_max, y_min)

markdown
Copiar
Editar
where:
- `x_max`: Maximum x-coordinate of the nonzero pixels.
- `x_min`: Minimum x-coordinate of the nonzero pixels.
- `y_max`: Maximum y-coordinate of the nonzero pixels.
- `y_min`: Minimum y-coordinate of the nonzero pixels.

# Details
1. Finds all nonzero pixel coordinates in the image.
2. Extracts x- and y-coordinates separately.
3. Computes the bounding box by determining the min and max values of each axis.
...
img[30:70, 40:80] .= 1.0  # Create a rectangular object in the image

bbox = get_bounding_box(img)
println("Bounding Box: ", bbox)
```
""" 
function get_bounding_box(img::AbstractArray)
    coords_ = findall(x -> x!=.0,img)
    xCoords::Vector{Int} = [x[1] for x in coords_]
    yCoords::Vector{Int} = [y[2] for y in coords_]
    return BoundingBox(
            maximum(xCoords),
            minimum(xCoords),
            maximum(yCoords),
...
"""
    filter_inHouse(img::AbstractArray, mockImage::AbstractArray, func::Function, kernel::Matrix{Int64}, size::Int64)::AbstractArray

Applies a custom filtering function to an image using a specified kernel. The function processes the image using a sliding window approach, applying `func` to the pixels where `kernel` is `true`.

# Arguments
- `img::AbstractArray`: The input image to be filtered.
- `mockImage::AbstractArray`: An array that will store the filtered output.
- `func::Function`: A function to apply to the selected pixels in the window.
- `kernel::Matrix{Int64}`: A binary matrix that defines the shape of the window to apply the function.
- `size::Int64`: The size of the window.

# Returns
- `AbstractArray`: The filtered image stored in `mockImage`.

# Details
1. A sliding window of size `(size, size)` is applied to `img`.
2. The `func` function is applied to the pixels selected by `kernel`.
3. The result is stored in `mockImage` using `mapwindow!`.

# Examples
```julia
using ImageFiltering

img = rand(100, 100)  # Generate a random 100x100 image
mockImage = similar(img)
kernel = [1 1 1; 1 1 1; 1 1 1]  # 3x3 kernel
filtered = filter_inHouse(img, mockImage, median, kernel, 3)
```
"""
function filter_inHouse(img::AbstractArray,mockImage::AbstractArray,func::Function,kernel::Matrix{Int64},size::Int64)
    function apply_function(window)
        values = window[Bool.(kernel)]
        return func(values)
    end
    return mapwindow!(apply_function,mockImage,img,(size,size))
end


"""
    nuclei_processing(img::AbstractArray)::Union{Matrix{Float64}, Array{Float64, 3}}

Processes a 3D image stack (or 2D image) to enhance nuclei by applying median filtering, Gaussian smoothing, and morphological operations.

# Arguments
- `img::AbstractArray`: A 2D or 3D grayscale image.

# Returns
- `Union{Matrix{Float64}, Array{Float64, 3}}`: The processed image of the same dimensions as `img`, with enhanced nuclei structures.

# Details
1. Computes `minR`, the characteristic size of nuclei.
2. Defines morphological kernels (`kernel1` and `kernel2`) based on `minR` and `minR * 2`.
3. Applies a median filter, followed by Gaussian smoothing.
4. Uses morphological min/max filters to refine structures.
5. Stores results in `result_img`, preserving the original image dimensions.

# Examples
```julia
using ImageFiltering

img = rand(100, 100, 5)  # Example 3D image stack
processed_img = nuclei_processing(img)
```
"""
function nuclei_processing(img::AbstractArray)::Union{Matrix{Float64}, Array{Float64, 3}}
    result_img::Array{Float64} = zeros(Float64, size(img))
    minR = getminR(img)
    println("MinR = ",minR)
    kernel1, size1 = drawKernel(minR)
    kernel2, size2 = drawKernel(minR*2)
    k_gauss = Kernel.gaussian(minR * 2)

    # Preallocate arrays to reuse
    # img_median = similar(img[:,:,1], Float64)
    # img_gauss = similar(img[:,:,1], Float64)
    # img_min = similar(img[:,:,1], Float64)
    # img_max = similar(img[:,:,1], Float64)
    # img_filtered = similar(img[:,:,1],Float64)

    for i = 1:size(img,3)
        boundingBox = get_bounding_box(img[:,:,i])
        img_median = similar(img[boundingBox.minX:boundingBox.maxX,boundingBox.minY:boundingBox.maxY,1], Float64)
        img_gauss = similar(img[boundingBox.minX:boundingBox.maxX,boundingBox.minY:boundingBox.maxY,1], Float64)
        img_min = similar(img[boundingBox.minX:boundingBox.maxX,boundingBox.minY:boundingBox.maxY,1], Float64)
        img_max = similar(img[boundingBox.minX:boundingBox.maxX,boundingBox.minY:boundingBox.maxY,1], Float64)
        filter_inHouse(img[boundingBox.minX:boundingBox.maxX,boundingBox.minY:boundingBox.maxY,i], img_median, median, kernel1, size1)
        imfilter!(img_gauss, img_median, k_gauss)
        filter_inHouse(img_gauss,img_min,minimum,kernel2,size2)
        filter_inHouse(img_min,img_max,maximum,kernel2,size2)
        result_img[boundingBox.minX:boundingBox.maxX,boundingBox.minY:boundingBox.maxY,i] = img_median .- img_max

        # filter_inHouse(img[:,:,i], img_median, median, kernel1, size1)
        # imfilter!(img_filtered, img_median, k_gauss)
        # filter_inHouse(img_filtered,img_filtered,minimum,kernel2,size2)
        # filter_inHouse(img_filtered,img_filtered,maximum,kernel2,size2)
        # result_img[:,:,i] = img_median .- img_filtered

        # Esta parte va sin inbound modification, más lenta.
        # img_proc = imfilter(img_median, Kernel.gaussian(minR*2))
        # img_proc = filter(img_proc,minimum,kernel2,size2)
        # img_proc = filter(img_proc,maximum,kernel2,size2)
        # img_proc = filter(filter(imfilter(img_median, Kernel.gaussian(minR*2)),minimum,kernel2,size2),maximum,kernel2,size2)
        # result_img[:,:,i] = img_median .- img_proc
...
function analyzeImage(path::String, rootpath::String, zparams::Union{Vector{Int},Nothing}=nothing, process::Int=0)
    image::Union{Matrix{Float64},Array{Float64,3},Array{Int,3},Array{Gray{N0f8},3},Array{Gray{N0f16},3}} = Gray.(load(path))
    if process == 1
        println("Processing image...")
        image = binarizeImage(nuclei_processing(image))
    else
        image = binarizeImage(Float64.(image))
    end
    if (typeof(image) == Array{Int,3})
        objc3d, df = ObjSplitter3D_fromBinaryImage(image,zparams)
        summ = summary_objs(objc3d)
        split(basename(path),".")[1]
        CSV.write("$rootpath\\"*split(basename(path),".")[1]*".txt", df, delim = '\t')
        CSV.write("$rootpath\\Summary_"*split(basename(path),".")[1]*".txt", summ, delim = '\t')
...
function startOSCAR()
    ...
    r::Bool = ask_dialog("Welcome to OSCAR!", "Select a folder", "Select a single file")
    path::String = ""
    ...
    try
        ...
        warn_dialog("Oops! Something went wrong while selecting the file or folder...")
        return  # Exit if there's an error
    end

    # Step 2: GUI - Create window with checkboxes and input fields
    ...
    function on_button_proceed_clicked(widget)
        ...
        if file_type == :tiff || file_type == :txt  # Single file selected
            if file_type == :txt
                ...
                if minZinterval < 1
                    ...
                    summary_df = summary_objs(objc3d, 1)
                    CSV.write("$base_dir\\Summary_$base_name.txt", summary_df, delim = '\t')
                    println("   >> " * string(length(objc3d.preok)) * " objects found.")
        ...
        elseif file_type == :folder  # Folder selected
            for file in readdir(path)
                ...
                if endswith(lowercase(file), ".txt")
                    ...
                    if isfile(matching_txt)
                        ...
                        println("No related TXT file found for " * file)
                    end
